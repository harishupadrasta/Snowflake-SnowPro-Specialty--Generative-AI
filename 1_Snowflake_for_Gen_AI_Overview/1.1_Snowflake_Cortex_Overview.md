...
- Powers **Snowflake Intelligence** (the built-in Snowsight experience)
...
|-----------|---------------|-------|
| AI Functions (AI_COMPLETE, AI_CLASSIFY, etc.) | **Per token** (input + output) | Both input and output tokens |
| AI_EMBED, AI_SIMILARITY | **Per token** (input only) | Only input tokens counted |
| AI_PARSE_DOCUMENT | **Per page** (970 tokens/page) | Document pages |
| AI_TRANSCRIBE | **50 tokens/second** of audio | Audio duration-based |
| AI_COUNT_TOKENS | **Compute only** | NO token charges (only warehouse cost) |
| Cortex Guard (guardrails) | **Additional input tokens** | Based on AI_COMPLETE output size |
| Cortex Search | **Per query + storage** (daily usage) | Serving + indexing |
| Cortex Analyst (direct API) | **Per message** | Successful responses only |
| Cortex Analyst (via Agents) | **Per token** | Billed like AI functions |
| Cortex Agents | **Per query** (includes tool calls) | Orchestration tokens |
| SPCS (custom models) | **Per compute-hour** (GPU instances) | Time-based, not token |
| Cortex REST API | **Dollars per million tokens** | Not credits — dollars |
| Cross-region inference | **Same credits as local** | No egress charges |

> **Exam Tip:** Credits are consumed in the **requesting region**, regardless of where cross-region inference processes the request. No data egress charges apply.
